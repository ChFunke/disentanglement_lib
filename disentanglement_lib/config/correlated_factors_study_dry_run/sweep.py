...
def get_plane_biases():
    ...
    return h.sweep("correlation_hyperparameter.bias_plane", h.discrete([0.0, 0.5, 1.0, 2.0, 10.0]))
...
class CorrelatedFactorsStudyDryRun(study.Study):
    ...
    def get_model_config(self, model_num=0):
        ...
        model_config_file = resources.get_file(
            "config/correlated_factors_study_dry_run/model_configs/shared.gin")
    ...
    def get_postprocess_config_files(self):
        ...
        return list(
            resources.get_files_in_folder(
                "config/correlated_factors_study_dry_run/postprocess_configs/"))

    def get_eval_config_files(self):
        """Returns evaluation config files."""
        return list(
            resources.get_files_in_folder(
                "config/correlated_factors_study_dry_run/metric_configs/"))
