...
def compute_dci(ground_truth_data, representation_function, random_state,
                ...
                batch_size=16):
    """Computes a modified version of the DCI scores after fast adaptation.
  ...
  """
    ...
    for num_labels in num_labels_available:
        ...
        pruned_importance_matrix = prune_importances(importance_matrix)
        ...
        scores[size_string + ":disentanglement"] = disentanglement(pruned_importance_matrix)
        scores[size_string + ":completeness"] = completeness(pruned_importance_matrix)

    scores['evaluation.name'] = "robust_dci_afa"
    return scores


def prune_importances(importance_matrix):
    max_importance_per_factor = np.amax(importance_matrix, axis=0)
    min_of_max_importance_per_factor = np.amin(max_importance_per_factor)
    importance_threshold = min_of_max_importance_per_factor / 2.
    threshold_indices = importance_matrix < importance_threshold
    importance_matrix[threshold_indices] = 0.0
...
def get_fast_adapted_representation_function(ground_truth_data, representation_function, random_state_for_few_labels,
    ...
    if num_labels_available == 0:
        ...
        def _fast_adapted_representation_function(x):
            ...
            representations[:, entangled_code_dimensions[0:len(corr_factors)]] = adapted_dimension_encodings[:, entangled_code_dimensions[0:len(corr_factors)]]
