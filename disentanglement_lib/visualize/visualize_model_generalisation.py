...
"""Visualization module for disentangled representations given a predefined observations."""
...
def visualize(model_dir,
    ...
    """Takes trained model from model_dir and visualizes it in output_dir starting from a predefined set of factors.

  Args:
    model_dir: Path to directory where the trained model is saved.
    output_dir: Path to output directory.
    factors_json: Path to json where predefined factors are saved as a list of lists for key 'factors_list'
  ...
  """
    ...
    with open(factors_json) as factors_json_file:
        factors_list = json.load(factors_json_file)['factors_list']
        corr_indices = gin.query_parameter("correlation_details.corr_indices")

        with hub.eval_function_for_module(module_path) as f:
            factors_batch = dataset.sample_factors(10, random_state)
            for factors_num, factors in enumerate(factors_list):
                factors_batch = fix_factors_batch(batch=factors_batch, factors=factors, fixed_indices=corr_indices)
                real_pics = dataset.sample_observations_from_factors(factors_batch, random_state)
                ...
                results_dir = os.path.join(output_dir, "reconstructions", str(factors_num))
                ...
                result = f(
                    dict(images=real_pics),
                    signature="gaussian_encoder",
                    as_dict=True)
                means = result["mean"]
                logvars = result["logvar"]
                results_dir = os.path.join(output_dir, "traversals", str(factors_num))
                if not gfile.IsDirectory(results_dir):
                    gfile.MakeDirs(results_dir)
                ...
                results_dir = os.path.join(output_dir, "animated_traversals", str(factors_num))
                if not gfile.IsDirectory(results_dir):
                    gfile.MakeDirs(results_dir)
                ...
                latents = f(
                    dict(images=real_pics), signature="gaussian_encoder", as_dict=True)["mean"]
                results_dir = os.path.join(output_dir, "interventional_effects", str(factors_num))
                vis_all_interventional_effects(factors_batch, latents, results_dir)
...
def fix_factors_batch(batch, factors, fixed_indices):
    for index in fixed_indices:
        batch[:, index] = factors[index]
    return batch
